...
void makeTree(Mat binaryImage,TreeDataType parentNode,CTree& docTree,int& totalNode,bool& tricker){
	// CTree docTree = CTree(1);	
	// int totalNode = 1;
	std::vector<Mat> childImages = cropImage(binaryImage,tricker);
	...
	if(childImages.size() > 1){
		for(std::vector<Mat>::const_iterator itc = childImages.begin();itc != childImages.end();itc++){
			...
			tricker = !tricker;
			makeTree(*itc,totalNode,docTree,++totalNode,tricker);
		}
	}else{		
		return;
	}
...
int main(){
	...
	makeTree(binaryImageReg,1,docTree,totalNode,tricker);
	
	/*
	std::vector<Mat> result = cropImage(binaryImageReg,true);
	for(std::vector<Mat>::const_iterator itm = result.begin();itm != result.end();itm++){
		// imwrite("konw.jpg",*itm);
		imshow("test",*itm);
		waitKey(-1);
	}
	*/
